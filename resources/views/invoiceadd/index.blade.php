    ...
    $.ajax({
        ...
        success: function(data){
			...
			$('#gridaccounttrans').edatagrid({
				...
				url:'/viewexp/'+$('#header').val(),
				method:'get',
				updateUrl:'/saveinvoicesexpedit',
				...
				onBeginEdit:function(rowIndex){
					...
					var header= $(editors[1].target);
					header.textbox('setValue',$('#header').val());
					var xpid= $(editors[2].target);
					xpid.textbox('setValue',$('#purchaseno').val());
					var inid= $(editors[3].target);
					inid.textbox('setValue',$('#typeahead').val());
					var six= $(editors[6].target);
					six.textbox({
						required:true,
					});
					var seven= $(editors[7].target);
					seven.textbox({
						required:true,
...
<center><h4> Purchase Expenses </h4></center>

<form id="barcodeform" method="POST" action="{{ url('editbarcodepurchases') }}" >
    <input  type="hidden" id="header" />
<input name="_token" type="hidden" value="{{ csrf_token() }}">
<div class="form-group">
<input type="text"  name="typeahead" value="{{$invoiceid}}" readonly class="typeahead tt-query" id="typeahead" placeholder="Bar code" />
...
<table class='easyui-datagrid' idField="accounttransid" rowNumbers='true' title='Expenses' iconCls='fa fa-table' singleSelect='true' url='' pagination='true' id='gridaccounttrans' method='get' fitColumns='true' style='width:100%' toolbar='#expensestoolbar'>
	...
	<th field="invoicexpid" hidden editor="numberbox"  >Invoice XpID</th>
	<th field="invoiceid" hidden editor="numberbox" >Invoice id</th>
...
$('#save').click(function(){
	...
	//window.location="/invoicereports";
        }
    });
