...
.datagrid-row-selected {
  ...
  color: white;
	...
	function validateform(){
		if($('#transdates').val()==''){
			$('#validdate').text('Date is required');
		}
		if($('#transdates').val()!=''){
			$('#validdate').text('');
		}
		if($('#payingaccount').val()==''){
			$('#validpayingaccount').text('Paying Account is required');
		}
		if($('#payingaccount').val()!=''){
			$('#validpayingaccount').text('');
		}
		if($('#mode').val()==''){
			$('#validmode').text('Mode of Payment is required');
		}
		if($('#mode').val()!=''){
			$('#validmode').text('');
		}
		
		if($('#mode').val()!='' && $('#payingaccount').val()!='' && $('#transdates').val()!=''  ){
			return true;
		}else{
			return false;
		}
	...
	$(function(){
        ...
        success: function(data){
			...
			$('#gridaccounttrans').edatagrid({
				...
				url:'/viewexp/'+$('#purchaseno').val(),
				method:'get',
				updateUrl:'/expenseedit',
...
<div class='col-lg-6'>
<div class='form-group'>
<div><label>Date</label>&nbsp;&nbsp;&nbsp;<b id="validdate" style="color:red;"> </b></div><div><input  class="easyui-datebox form-control"  name='transdates' 
 id='transdates' required="true" style="width:100%;height:34px" data-options="onSelect:validateDate"   /></div>
</div>
</div>
<div class='col-lg-6'>
<div class='form-group'>
<div><label>Mode of Payment</label>&nbsp;&nbsp;&nbsp;<b id="validmode" style="color:red;"> </b></div><input  class='form-control easyui-combobox' style="width:100%;height:34px"   name='mode' 
 id='mode' /></div>
</div>

<div class="col-lg-6">
	<div class="form-group">
		<div><label>Paying Account</label>&nbsp;&nbsp;&nbsp;<b  style="color:red" id="validpayingaccount"> </b></div>
...
<table class='easyui-datagrid' idField="accounttransid" rowNumbers='true' title='Expenses' iconCls='fa fa-table' singleSelect='true' url='' pagination='true' id='gridaccounttrans' method='get' fitColumns='true' style='width:100%' toolbar='#expensestoolbar'>
	<thead><tr>
	<th field="_token" editor="textbox" hidden >_token</th>
	<th field="purchaseno" editor="numberbox" hidden  >purchaseno</th>
	<th field='id' width='100'  hidden>id</th>
	<th field='accountcode' hidden width='100' editor="numberbox"  >Accountcode</th>
	<th field='payingaccount'  hidden editor="numberbox" >PayingAccount</th>
...
onSelect:function(rows){
	...
	<th field='ttype' width='100' hidden >ttype</th>
	<th field="date" editor="textbox" hidden  >Date</th>
	<th field="Aid"  editor="textbox" hidden  >Aid</th>
