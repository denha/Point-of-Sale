...
<table class='easyui-datagrid'  striped='true' rowNumbers="true" title='Stock Recieved' iconCls='fa fa-table' showFooter="true" singleSelect='true' url='' pagination='true' id='gridcustomers'  fitColumns='true' style='width:100%' toolbar='#customerstoolbar'>
<thead><tr>
<th field="id" hidden> ID</th>
<th field='date' width='100'>Date</th>
<th field='purchaseNo' width='80'>Trans #</th>
<th field='name' width='180'>Itemname</th>
<th field='remarks' width='100'>Remarks</th>
<th field='qty' width='80'>Qty</th>
<th field="saveid" hidden width="100">SaveID</th>
</tr></thead>
</table>
...
method:'get',
url:'/viewrecieved?end='+finend+'&start='+finstart,
...
if(a==null){
    ...
    $.messager.confirm('Confirm','Are you sure you want to Delete this Record',function(r){
        if(r){
            var row=$('#gridcustomers').datagrid('getSelected');
            $.ajax({
             url:'/delrecievedstoc/'+row.id,
             method:'POST',
             data:{'saveid':row.saveid,'purchaseNo':row.purchaseNo,'qty':row.qty,'_token':$('input[name=_token]').val()},
             success:function(){
                $('#gridcustomers').datagrid('reload');
             }
            });
...
$('#excel').click(function(){
    ...
    var date2=$('#date2').val();
    if(date1=='' && date2==''){
        window.location="stockexcel/xlsx/today/today";
    }
    else if(date1!='' && date2==''){
        window.location="stockexcel/xlsx/"+date1+"/today";  
    }
    else if(date1!='' && date2!=''){
        window.location="stockexcel/xlsx/"+date1+"/"+date2;  
    }
