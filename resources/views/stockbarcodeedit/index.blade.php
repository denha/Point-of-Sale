...
if($first=='first'){
$yes=$cartid;
DB::delete("delete from tbl_cart");
DB::statement(" insert into tbl_cart (product_id,quantity,sellingpx,header,stockid) select stockname,quantity,stocktrans.sellingrate/stocktrans.quantity as selllingrate,purchaseheaderid,stocktrans.id from stocktrans inner join stocks on stocks.id=stocktrans.stockname where purchaseheaderid=$yes");
}else{
...
foreach($items as $ite){
  ...
  $shoppingCart->addToCart($productResult[0]["id"], 1, $member_id,$ite->sellingrate,0);
		...
		echo "My";
            if (! empty($_GET["quantity"])) {
                ...
                if (! empty($cartResult)) {
                    ...
                    // Add to cart table
                    $shoppingCart->addToCart($productResult[0]["id"], $_GET["quantity"], $member_id);
                }
            }
            break;
        case "remove":
...
?>
<input type="text" id="number"  hidden  value="{{$cartid}}" />
<center><h4> Edit Sales </h4></center>

<form id="barcodeform" method="POST" action="{{ url('/editchanges').'/'.$cartid }}" >
...
<input class="easyui-textbox" id="transdates" style="width:56%;height:34px;" value="{{$det->transdate}}" />
<input  class="easyui-combobox" style="width:56%;height:34px;" value="{{$det->customer}}" id="custo" />
...
<label> <b style="color:grey">CASH PAID  :</b></label><input type="text" value="
<?php
  if (! empty($cartItem)) { 
 ...
 ?>"
 id="cashpaid" name="cashpaid" style="width:170px;margin-left:89px;" /><br>
<label> <b style="color:grey">BAL  :</b></label><input value=0 id="bal" type="text" style="width:170px;margin-left:136px;" />
...
function save_to_db(cart_id, new_quantity, newPrice,pric) {
    ...
    $.ajax({
		...
		success : function(response) {
            ...
            $("#cashpaid").val(totalItemPrice.toLocaleString());
		}
	});
}
...
$('.remove').click(function(){
  ...
  var edit='yes';
  var info = 'id=' + del_id+"&_token="+$('input[name=_token]').val()+"&edit=Yes";
  $.ajax({
           type : "POST",
           url : "/delbarcodeitem", 
           data : info,
           success : function() {
           // $('#dateform').submit();
           window.location="/editbarcodesales/"+{{$cartid}}+"/second";
                        }
          });
...
//alert(cashpaid);
$.ajax({
'url':'/editsalesconvert',
'method':'POST',
'data':{'customer':custo,'transdates':transdates,'bal':bal,'totalcost':totalcost,'number':number,'cashpaid':cashpaid,'_token':$('input[name=_token]').val()},
success:function(data){
  window.location="/barcodesales";//"/editbarcodesales/"+{{$cartid}}+"/second";
