...
DB::statement(" insert into tbl_cart (product_id,quantity,sellingpx,header,stockid) select stockname,quantity,stocktrans.buyingrate as selllingrate,purchaseheaderid,stocktrans.id from stocktrans inner join stocks on stocks.id=stocktrans.stockname where purchaseheaderid=$yes");
}else{
...
<center><h4> Edit Purchases </h4></center>

<form id="barcodeform" method="POST" action="{{ url('/changePurchases').'/'.$cartid }}" >
...
$('.remove').click(function(){
  ...
  $.ajax({
           ...
           success : function() {
           // $('#dateform').submit();
           window.location="/editbarcodepurchase/"+{{$cartid}}+"/second";
                        }
          });
...
//alert(cashpaid);
$.ajax({
'url':'/editpurchaseconvert',
'method':'POST',
'data':{'supplier':custo,'transdates':transdates,'bal':bal,'totalcost':totalcost,'number':number,'cashpaid':cashpaid,'_token':$('input[name=_token]').val()},
success:function(data){
 window.location="/purchaseheaders";
