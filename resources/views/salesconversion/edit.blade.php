...
if (isset($_POST['changeprice']) && isset($_POST['realquantity']) ){
  if(!empty($_SESSION["shopping_cart"])) {
  foreach($_SESSION["shopping_cart"] as &$value){
$_POST['code'];
    if($value['code'] == $_POST['code']){
...
<td width="150px"><?php echo number_format($product["sellingrate"]*$product['realquantity'],0); ?></td>
<td width="150px" >
...
<?php
$total_price += ($product["sellingrate"]*$product["realquantity"]);
}
}
?>
</table>
...
$a=$a+$product['sellingrate']*$product['realquantity'];
    }else{
$b=$b+$product['sellingrate']*$product['realquantity'];
    }
