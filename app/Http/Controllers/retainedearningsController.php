...
public function incomestatepdf($date1,$date2,$day1,$day2){
    ...
    $totalexpense=DB::select("select  format(sum(if(ttype='D',amount,-amount)),0) as amount,ttype,accountname,bracid,accountcode from expensepdfs where bracid=$branch and transdate between '$date1' AND '$date2' $thedate");
    $expense=DB::select("select format(sum(if(ttype='D',amount,-amount)),0) as amount ,ttype,accountname,bracid,accountcode from expensepdfs where bracid=$branch and transdate between '$date1' AND '$date2' $thedate group by accountcode");
