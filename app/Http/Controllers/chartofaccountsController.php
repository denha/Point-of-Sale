...
public function viewcombo($id){
   ...
   // return chartofaccounts::where('accounttype',$id)->where('branchno','=',$bra)->get();
    return DB::select("select *,isDefault as defaul,if(isDefault=1,'Yes','No') as isDefault from chartofaccounts where accounttype=$id  and branchno=$bra and accountcode!='1111' AND accountcode!='1113' ");
}
...
public function inventoryaccounts(Request $request,$id){
    DB::beginTransaction();
    try{
        $bra=auth()->user()->branchid;
        DB::update("update chartofaccounts set isDefault=0,isInventory=0");
$Objchartofaccounts=chartofaccounts::find($id);
$Objchartofaccounts->isDefault=$request['isDefault'];
$Objchartofaccounts->isInventory=1;
$Objchartofaccounts->save();
