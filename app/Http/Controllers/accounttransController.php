...
//$Objaccounttrans->stockidentify=56;
$Objaccounttrans->bracid=auth()->user()->branchid;
$Objaccounttrans->save();
//Debiting remember its a double entry
$Objaccounttrans1= new accounttrans();
$Objaccounttrans1->accountcode=$request['payingaccount'];//Fixed Account for sale of goods
...
//$Objaccounttrans1->stockidentify=5;
...
public function expenseedit(Request $request ){
    ...
    try{
        //Expense Account
    accounttrans::where('id','=',$id)->where('ttype','=','D')->update(['amount'=>$request['amount'],'total'=>$request['amount'],'narration'=>$request['narration'],'accountcode'=>$request['accountcode']]);//->update(['narration'=>$request['naration']]);
    // Crediting Cash Account
    accounttrans::where('id','=',$id+1)->where('ttype','=','C')->update(['amount'=>$request['amount'],'total'=>$request['amount']*-1,'narration'=>$request['narration'],'accountcode'=>$request['payingaccount']]);//->update(['narration'=>$request['naration']]);
    }catch(\Exception $e){
        DB::rollback();
        echo "Failed ".$e;
    }
...
public function incomeedit(Request $request ){
    ...
    try{
        //Expense Account
    accounttrans::where('id','=',$id+1)->where('ttype','=','D')->update(['amount'=>str_replace( ',', '',$request['amount']),'total'=>str_replace( ',', '',$request['amount']),'narration'=>$request['narration'],'accountcode'=>$request['payingaccount']]);//->update(['narration'=>$request['naration']]);
    // Crediting Cash Account
    accounttrans::where('id','=',$id)->where('ttype','=','C')->update(['amount'=>str_replace( ',', '',$request['amount']),'total'=>str_replace( ',', '',$request['amount']),'narration'=>$request['narration'],'accountcode'=>$request['accountcode']]);//->update(['narration'=>$request['naration']]);
