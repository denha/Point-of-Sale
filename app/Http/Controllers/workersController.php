...
public function bsheetpdf($date1,$date2,$date3){
    ...
    $totalexpense=DB::select("select  format(sum(if(ttype='D',amount,-amount)),0) as amount,ttype,accountname,bracid,accountcode from expensepdfs where bracid=$branch and transdate between '$date1' AND '$date2'  AND transdate <='$finaldate'");
    ...
    $equity=DB::select(" SELECT accountname,accounttype,accountcode,format(if(ttype='D',sum(-Debit),sum(Debit)),0)  as amount FROM `bsheetpdfs` where transdates between '$date1' AND '$date2'  AND transdates <='$finaldate' and bracid=$branch and accounttype=5 group by accountcode");
    $totalequity=DB::select(" SELECT accountname,accounttype,accountcode,format(if(ttype='D',sum(-Debit),sum(Debit)),0)  as amount FROM `bsheetpdfs` where transdates between '$date1' AND '$date2'  AND transdates <='$finaldate' and bracid=$branch and accounttype=5");
    $pdf = \App::make('dompdf.wrapper');
...
public function viewcode($id){
    return view('viewcode/index')->with('id',$id);
}
public function expensepreview($date1,$date2){
   ...
   $expense= DB::select("select *,format(amount,0) as amount,DATE_FORMAT(transdate,'%d-%m-%Y') as transdate from expensepdfs  where  amount>0 and accountcode!=1001 and $where order by transdate asc ");
   $expensetotal= DB::select("select format(sum(amount),0) as amount from expensepdfs where  accountcode!=1001 and $where ");
    $pdf = \App::make('dompdf.wrapper');
    $da=date('d-m-Y');
...
public function customerled($date1,$date2,$id){
    $branch=auth()->user()->branchid;
    $company=DB::select("select * from companys where id=$branch");
    $where='';
    $asof='';
    if($date1=='today' && $date2==0){
        $today=date("Y-m-d");
        $asof="As of ".date("d-m-Y");
        $where="transdate<='$today' and customer=$id";
    }
    else if($date1!="" && $date2==0){
        $asof="As of ".date("d-m-Y", strtotime($date1));
        $finaldate=date("Y-m-d", strtotime($date1));
        $where="transdate<='$finaldate' and customer=$id";
    }
    else if($date1!='' && $date2!=''){
        $asof="From ".date("d-m-Y", strtotime($date1))." To ".date("d-m-Y", strtotime($date2));
        $finaldate1=date("Y-m-d", strtotime($date1));
        $finaldate2=date("Y-m-d", strtotime($date2));
        $where="transdate BETWEEN '$finaldate1' AND '$finaldate2' and customer=$id ";
    }
    $clients=DB::select("select * from customers where id=$id");
   $expense= DB::select("select stocks.name as stocknames,quantity, customers.id as custoid,DATE_FORMAT(transdate,'%d-%m-%Y') as transdate,customers.name as customer, purchaseheaderid, format(totalamt,0) as totalamt, format(totalamt-totalpaid,0) as bal,  format(totalpaid,0) as totalpaid, stockname FROM `stocktrans` inner join customers on customers.id= stocktrans.customer inner join stocks on stocks.id=stocktrans.stockname where $where order by stocktrans.id ");
   $expensetotal= DB::select("select  format(sum(totalamt),0) as totalamt, format(sum(totalamt)-sum(totalpaid),0) as bal,  format(sum(totalpaid),0) as totalpaid, stockname FROM `stocktrans` inner join customers on customers.id= stocktrans.customer inner join stocks on stocks.id=stocktrans.stockname where $where");
    $pdf = \App::make('dompdf.wrapper');
    $da=date('d-m-Y');
    $pdf->loadHTML(view('customerledgerpdfs/index')->with('company',$company)->with('expense',$expense)->with('expensetotal',$expensetotal)->with('asof',$asof)->with('client',$clients));
    return $pdf->stream();    
}
public function salescredits($date1,$date2){
    $branch=auth()->user()->branchid;
    $company=DB::select("select * from companys where id=$branch");
    $where='';
    $asof='';
    ...
    //$clients=DB::select("select * from customers where id=$id");
   $expense= DB::select("select customers.id as custoid,DATE_FORMAT(transdate,'%d-%m-%Y') as transdate,name as customer, purchaseheaderid,sum(totalamt) as  totalamt, format(sum(totalamt)-sum(totalpaid),0) as bal, sum(totalpaid) as totalpaid, stockname FROM `stocktrans` inner join customers on customers.id= stocktrans.customer where $where  group by purchaseheaderid having totalpaid<totalamt  ");
   ...
   $expensetotal= DB::select("SELECT format(sum(bal),0) as bal from pendings");
    $pdf = \App::make('dompdf.wrapper');
    $da=date('d-m-Y');
    $pdf->loadHTML(view('salescreditpdfs/index')->with('company',$company)->with('expense',$expense)->with('expensetotal',$expensetotal)->with('asof',$asof));
    return $pdf->stream();    
}
}