...
use Illuminate\Support\Facades\DB;
 ...
 class accountingreportsController extends Controller{
   ...
   if(isset($_GET['page'])&& isset($_GET['rows']) && !empty($_GET['start']) && !empty($_GET['end']) && !empty($_GET['date1']) && !empty($_GET['date2']) &&isset($_GET['account'])){   
    ...
    public function viewcodeaccount($id){
      $today=date("'Y/m/d'"); 
      if($id==3600){
      $this->report("where transdates = $today  AND accounttrans.accountcode=$id AND amount>0 ","sum(if(ttype='D',amount,''))as amount,format(sum(if(ttype='D',amount,'')),0) as debits,format(sum(if(ttype='C',amount,'')),0) as credits "," ");
      }else{
         $this->report("where  accounttrans.accountcode=$id AND amount>0 ","sum(if(ttype='D',amount,''))as amount,format(sum(if(ttype='D',amount,'')),0) as debits,format(sum(if(ttype='C',amount,'')),0) as credits "," ");   
      }
    }
